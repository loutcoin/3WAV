...
contract WavExchangeToken is Test {
    ...
    function testWavExchangeCContentTokenHappyPath() public {
...

        {
            ...
            /*bytes memory data = abi.encodePacked(
                buyer_02,
                _wavResale.seller,
                _wavResale.creatorId,
                _wavResale.hashId,
                _wavResale.numToken,
                _wavResale.purchaseQuantity,
                _wavResale.priceInEth,
                NONCE
            );

            (uint8 v, bytes32 r, bytes32 s) = vm.sign(ownerKey, data);
            ...
            //(uint8 v, bytes32 r, bytes32 s) = vm.sign(privateKey, _messageHash)
            /*(uint8 v, bytes32 r, bytes32 s) = vm.sign(ownerKey, data);

            bytes32 vs = bytes32(uint256(s) | (uint256(v - 27) << 255));

            bytes memory signature64 = abi.encodePacked(r, vs);*/
        ...
        (
            ...
            uint16 _num,
            uint256 _bal
        ) = WavAccess(address(wavDiamond)).returnOwnershipIndex(buyer_02, 0);
    }
    ...
    // testWavExchangeSContentTokenHappyPath
