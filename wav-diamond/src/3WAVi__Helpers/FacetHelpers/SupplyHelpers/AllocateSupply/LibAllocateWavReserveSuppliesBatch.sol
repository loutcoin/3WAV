...
library LibAllocateWavReserveSuppliesBatch {
    error AllocateWavReserveSuppliesBatch__LengthValIssue();
        ...
        for (uint256 i = 0; i < _wavSaleToken.length; ++i) {
...

            {
                ...
                if (_sCTKN.supplyVal != 0) {
                    ...
                    if (_wST.numToken == 0) {
                        (uint112 _totalSupply, , , ) = SupplyDBC
                            ._cSupplyValDecoder(_sCTKN.supplyVal);
            ...
            // Branch 3: CContentToken (numToken != 0)
            {
                if (_wST.numToken != 0) {
                    ...
                    uint256 _packed = SupplyMap.s_tierMap[_wST.hashId][
                        _wordIndex
                    ];
