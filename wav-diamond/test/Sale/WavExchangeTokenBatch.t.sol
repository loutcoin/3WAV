...
contract WavExchangeTokenBatch is Test {
    ...
    function testWavExchangeCContentTokenBatchHappyPath() public {
...

        {
            ...
            bytes32 messageHash = keccak256(
                abi.encodePacked(_nonce, owner, _valueRequired)
            );
        ...
        bytes memory _payload = abi.encodeWithSelector(
            WavAccess(address(wavDiamond)).returnOwnership.selector,
            buyer_02
        );
    ...
    // forge test --match-test testWavExchangeCVariantBatchHappyPath -vvvv
